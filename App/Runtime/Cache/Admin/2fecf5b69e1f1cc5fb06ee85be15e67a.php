...
<!-- S -->
<form class="form-horizontal form-search" role="form" method="get" action="/index.php/Admin/Link/index.html">
	<div class="my-table-tool" id="my-table-tool">
		...
		<input type="text" name="keyword" value="<?php echo ((isset($_GET['keyword']) && ($_GET['keyword'] !== ""))?($_GET['keyword']):''); ?>" placeholder="请输入关键词" class="nav-search-input input-sm" id="nav-search-input" autocomplete="off"">
					...
					<table id="data-table" class="table table-bordered table-hover">
						...
						<tbody>
							...
							<?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
									...
									<td class="center">
										<a title="<?php echo ($vo["title"]); ?>" target="_blank" href="<?php echo ((isset($vo["url"]) && ($vo["url"] !== ""))?($vo["url"]):'javascript:void(0);'); ?>"><?php echo (cutStr($vo["title"],20)); ?></a>
									</td>
									...
									<td class="center cursor-pointer">
										<span data-id="<?php echo ($vo["id"]); ?>" class="label label-sm status 
											<?php if(($vo["status"]) == "1"): ?>label-success">显示<?php else: ?>label-grey">隐藏<?php endif; ?>
										</span>
									</td>
									<td class="center"><?php echo (getAdminUser($vo["update_id"],1)); ?></td>
									<td class="center"><?php echo (getAdminUser($vo["create_id"],1)); ?></td>
    ...
    jQuery(function ($) {
		...
		$('.my-footer-tool').on('click', '.alldel', function() {
			...
			if (inputs.length && confirm('确定删除已选择的数据？')) {
				...
				// 将数据传到后台处理
				$.ajax({
					url: "<?php echo U('del',array('type'=>'alldel'));?>",
					type: 'GET',
					data: {"id": ids},
					dataype: 'json',
