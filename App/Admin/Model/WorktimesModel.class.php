...
{	
	...
	{	
		...
		$data['data'] = $this->alias('a')
						->join(" RIGHT JOIN `".C('DB_PREFIX')."admin` b ON a.`admin_id` = b.`id`")
						->join(" INNER JOIN `".C('DB_PREFIX')."group` c ON b.`group_id` = c.`id`")
						->where($where)->order('c.sort ASC')
						->select();
		$thisMonth = date('Y-m-01');
		$maxMonth = date('d', strtotime(date('Y-m-01')." +1 month -1 day"));
		$weeks = C('WEEKS');
		$weeksData = array();
		for($i=1; $i<= $maxMonth; $i++){
			$thisWeek = date('w', strtotime(date('Y-m-', strtotime($thisMonth)).str_pad($i,2,"0",STR_PAD_LEFT)));
			$weeksData[$i]['day'] = $i.'日';
			$weeksData[$i]['week'] = $weeks[$thisWeek];
		}
